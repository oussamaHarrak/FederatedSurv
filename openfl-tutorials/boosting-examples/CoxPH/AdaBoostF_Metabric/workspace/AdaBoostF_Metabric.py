...
def validate_adaboost(model, val_loader, device, name):
    ...
    if LOG_WANDB:
         wandb.log({"Model Concordance index": c_index_value , 
                    "Model Integrated_Brier_score" : integrated_brier_score_value},
                  commit=False)
  
    return {'Corcondance-index': c_index_value ,  'Integrated-Brier-Score' : integrated_brier_score_value}

wandb.init(project="Federated_Learning_metabric",settings=wandb.Settings(_service_wait=360))
