...
def train_adaboost(model, train_loader, device, optimizer, adaboost_coeff, name):
 ...
 #   survs = weak_learner.predict_survival_function(X)
    #min_Survival = y['Survival'].min()
    #max_Survival = y['Survival'].max()
    #print('min_Survival :' , min_Survival)
    #print('max_Survival : ', max_Survival)
    y_pred = weak_learner.predict(X)
  #  Survivals = np.arange(min_Survival + 2  , max_Survival + 1 )
 ...
 #   Survivals = np.arange(4.0 , 270.0)
    ...
    for idx, weak_learner in enumerate(model.get_estimators()):
       # preds = np.asarray([[fn(t) for t in Survivals] for fn in survs])
        ...
        c_index_value = c_index_value[0]
        error.append(adaboost_coeff[idx]*(1- c_index_value)) #Calculate the weighted error for each weak learner 1-c_index_value represents the error
    ...
    error.append(adaboost_coeff.sum() * (1-np.mean(c_index)))
    wandb.log({"Concordance_indexes" : c_index})
...
def validate_adaboost(model, val_loader, device, name):
 ...
 #   Survivals = np.arange(4.0 , 270.0)
  #  preds = np.asarray([[fn(t) for t in Survivals] for fn in survs])
  #  brier_score = brier_score(y_structured,y_structured,preds,Survivals)
...
federation = Federation(client_id=client_id, director_node_fqdn=args.server, director_port='50055', tls=False) #Changed from to 50054
