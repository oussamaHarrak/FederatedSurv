...
class veteran_ShardDataset(ShardDataset):
    ...
    def __init__(self, x, y, data_type, rank=1, worldsize=1, complete=False):
        ...
        #quantitySkewSplitter = QuantitySkewSplitter(alpha = 2)
        #data_collaborators = quantitySkewSplitter.split(x,y,self.worldsize)
        self.x = x if complete else  x[self.rank - 1::self.worldsize] #x[self.rank - 1::self.worldsize]
        self.y = y if complete else  y[self.rank - 1::self.worldsize] #y[self.rank - 1::self.worldsize]
...
class veteran_ShardDescriptor(ShardDescriptor):
    ...
    def __init__(
        ...
        self.train_idx = train_splitter.split(y_train, self.worldsize)
        self.test_idx = test_splitter.split(y_test, self.worldsize)
        self.data_by_type = {
