...
class Fl_Chain_ShardDescriptor(ShardDescriptor):
    ...
    def download_data(self):
        ...
        #Preprocessing of the nan values
        mean_creatinine = np.mean(X.loc[:,'creatinine'])
        X.loc[:,'creatinine'].fillna(mean_creatinine , inplace = True)
        X.loc[:,'chapter'] = X.loc[:,'chapter'].astype(str)
        X.loc[:,'chapter'].fillna('Missing', inplace=True)
        #Label Encoding of the categorical columns 
        qualitative_cols = X.select_dtypes(exclude=['int', 'float'])
        qualitative_cols = qualitative_cols.columns
        ...
        """
        #X = pd.read_csv('../fl_chain_dataset/X.csv' , sep = ",")
        #y = pd.read_csv('../fl_chain_dataset/y.csv' , sep = ",")
        df = pd.read_csv('../fl_chain_dataset/FLChain.csv' , sep = ",")
        df = df.iloc[:,1:]
        X = df.drop(columns = ['futime','death'], axis=1)
        y = df[['futime' , 'death']]
