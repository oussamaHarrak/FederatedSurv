...
def validate_adaboost(model, val_loader, device, name):
    ...
    time = y['futime']
    y_structured = np.array([(e, t) for e, t in zip(event_indicator, time)], dtype=[('death', bool), ('futime', float)])
...
wandb.init(project="Federated_Learning_metabric",settings=wandb.Settings(_service_wait=120))
