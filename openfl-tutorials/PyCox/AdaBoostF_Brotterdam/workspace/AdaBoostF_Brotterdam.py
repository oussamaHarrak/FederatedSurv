...
def train_adaboost(model, train_loader, device, optimizer, adaboost_coeff, name):
    ...
    y_structured = np.array([(e, t) for e, t in zip(event, durations)], dtype=[('event', bool), ('durations', float)])
    ...
    surv_array = weak_learner.predict_survival_function_(X,y)
    ...
    print("surv_array after interpolation:" , surv_array)
    integrated_brier_score_value = integrated_brier_score(y_structured, y_structured, surv_array, event_times)
...
wandb.init(project="Federated_Learning_brotterdam",settings=wandb.Settings(_service_wait=120))
