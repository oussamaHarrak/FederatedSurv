...
class brotterdam_ShardDataset(ShardDataset):
    ...
    def __init__(self, x, y, data_type, rank=1, worldsize=1, complete=False):
        ...
        #splitter = DirichletNumPyDataSplitter(alpha=0.5, min_samples_per_col=10, seed=0)
        ...
        print(f"len(x) envoy {self.rank} : {len(self.x)}" )
        print(f"len(y) envoy {self.rank} : {len(self.y)}")
        print("isnan(X) :" , np.sum(np.isnan(x)))
...
class brotterdam_ShardDescriptor(ShardDescriptor):
    ...
    def get_dataset(self, dataset_type='train', complete=False):
        ...
        return brotterdam_ShardDataset(
    ...
    def dataset_description(self) -> str:
        ...
        return (f'brotterdam dataset, shard number {self.rank}'
    ...
    def download_data(self):
        df_train = pd.read_csv('../brotterdam_dataset/Brotterdam.csv' , sep = ",")
        df_train = df_train.iloc[:,1:]
        df_test = df_train.sample(frac=0.2)
        df_train = df_train.drop(df_test.index)
        cols_standardize =['age', 'nodes' , 'pgr' , 'er']
        cols_leave =  ['meno' , 'size' , 'grade','hormon']
